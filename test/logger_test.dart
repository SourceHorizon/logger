...
void main() {
  ...
  test('Logger.log', () {
    var logger = Logger(filter: NeverFilter(), printers: [callbackPrinter]);
    logger.log(Level.debug, 'Some message');

    expect(printedMessage, null);

    logger = Logger(filter: AlwaysFilter(), printers: [callbackPrinter]);
  ...
  test('Logger.t', () {
    var logger = Logger(filter: AlwaysFilter(), printers: [callbackPrinter]);
    var stackTrace = StackTrace.current;
    logger.t('Test', error: 'Error', stackTrace: stackTrace);
    expect(printedLevel, Level.trace);
    expect(printedMessage, 'Test');
